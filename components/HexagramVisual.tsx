      ...
      {lines.map((isYang, index) => {
        ...
        return (
          <div key={index} className="w-full flex items-center gap-2 group relative">
            {/* Line UI */}
            <div className={`flex-1 ${itemHeight} flex items-center justify-between`}>
              {isYang === 1 ? (
                /* Yang Line */
                <div className={`w-full ${itemHeight} bg-slate-800 rounded-sm ${isChanging ? 'ring-2 ring-amber-500 ring-offset-1' : ''}`}></div>
              ) : (
                /* Yin Line */
                <>
                  <div className={`w-[45%] ${itemHeight} bg-slate-800 rounded-sm ${isChanging ? 'ring-2 ring-amber-500 ring-offset-1' : ''}`}></div>
                  <div className={`w-[45%] ${itemHeight} bg-slate-800 rounded-sm ${isChanging ? 'ring-2 ring-amber-500 ring-offset-1' : ''}`}></div>
                </>
              )}
            </div>
